...
#include "caffe/layers/upsample_layer2.hpp"

namespace caffe {

	template <typename Dtype>
	void Upsample2Layer<Dtype>::Forward_cpu_bilinear(
								 Blob<Dtype>* bottom,
								 Blob<Dtype>* top) {
	...
	template <typename Dtype>
	void Upsample2Layer<Dtype>::Forward_cpu_nearest(
								 Blob<Dtype>* bottom,
								 Blob<Dtype>* top) {
		const Dtype* bottom_data = bottom->cpu_data();
		Dtype* top_data_mu = top->mutable_cpu_data();
	...
	template <typename Dtype>
	void Upsample2Layer<Dtype>::LayerSetUp(
								const vector<Blob<Dtype>*>& bottom,
								const vector<Blob<Dtype>*>& top) {
		Upsample2Parameter upsample_param = this->layer_param_.upsample2_param();
		if (2 != bottom.size() && 
	...
	template <typename Dtype>
	void Upsample2Layer<Dtype>::Reshape(
								const vector<Blob<Dtype>*>& bottom,
								const vector<Blob<Dtype>*>& top) {
		Upsample2Parameter upsample_param = this->layer_param_.upsample2_param();
	...
	template <typename Dtype>
	void Upsample2Layer<Dtype>::Forward_cpu(
								const vector<Blob<Dtype>*>& bottom,
								const vector<Blob<Dtype>*>& top) {
		if(Upsample2Parameter_UpsampleOp_NEAREST == this->layer_param_.upsample2_param().mode()) {
			Forward_cpu_nearest(bottom[0],top[0]);
		}
		else if(Upsample2Parameter_UpsampleOp_BILINEAR == this->layer_param_.upsample2_param().mode()) {
			Forward_cpu_bilinear(bottom[0],top[0]);
		}
	}

	template <typename Dtype>
	void Upsample2Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
	STUB_GPU(Upsample2Layer);
#endif

	INSTANTIATE_CLASS(Upsample2Layer);
	REGISTER_LAYER_CLASS(Upsample2);
}// namespace caffe
